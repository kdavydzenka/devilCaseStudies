...
if (row$continuous) {
  for (i in 2:ncol(design_matrix)) {
    design_matrix[,i] = design_matrix[,i] + rnorm(nrow(design_matrix), 0, sd = .2)
  }
}
...
# Fit devil
s = Sys.time()
devil.fit = devil::fit_devil(input_matrix, design_matrix, size_factors = NULL, parallel.cores = 1)
