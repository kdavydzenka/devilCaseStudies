...
#path.tail <- paste0(author, "_", idx, "dupCorr.csv")
dir.create("results")

saveRDS(timing_df, paste0("timing_results/", author, "_", idx, "_dupCorr.rds"))
saveRDS(df.result, paste0("results/", author, "_", idx, "_dupCorr.rds"))
...
df %>%
  ...
  dplyr::mutate(
    ...
    MCC = num / den,
    F1 = 2 * TP / (2 * TP + FP + FN)
  ) %>% dplyr::select(name, TP, FP, FN, MCC, F1) %>% distinct() %>%
  dplyr::arrange(-MCC)
