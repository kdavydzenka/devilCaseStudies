...
#df.null <- t(df.result)[(max_gene+1):MAX_GENE,]
#df.pow <- t(df.result)[1:max_gene,]

# save name
#is.pb = this$is.pb
path.tail <- paste0(author, "_", idx, ".csv")
dir.create("results")

saveRDS(timing_df, paste0("timing_results/", author, "_", idx, ".rds"))
saveRDS(df.result, paste0("results/", author, "_", idx, ".rds"))
