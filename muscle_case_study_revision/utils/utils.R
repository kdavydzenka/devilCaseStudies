...
fit_de <- function(input_data, method = "devil",design_type = "age_only") {
  ...
  if (method == 'devil') {
    ...
    if (is.null(data_g)) {
      fit <- nebula::nebula(counts, id = metadata$patient, pred = design_matrix, offset = sf, covariance = TRUE, ncore = 1)
    } else {
      fit <- nebula::nebula(data_g$count, id = data_g$id, pred = data_g$pred, offset = data_g$offset, covariance = TRUE, ncore = 1)  
    }
  }
...
de_test <- function(input_data,
  ...
  if (method == 'devil') {
    ...
    n_variables = sum(grepl("logFC", colnames(fit_res$summary)))
    ...
    for (gene_i in seq_len(n_genes)) {
      ...
      eff <- sum(contrast * fit_res$summary[gene_i, 1:n_variables])
